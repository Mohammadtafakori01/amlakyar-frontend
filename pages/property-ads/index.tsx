...
export default function PropertyAdsPage() {
  ...
  // Initialize - fetch cities and published ads
  useEffect(() => {
    if (!hasFetched.current) {
      ...
      const initialFilters: PropertyAdFilters = {
        ...localFilters,
        status: AdStatus.PUBLISHED, // Default to published ads for public view
        page: 1,
        limit: pageSize,
      };
  ...
  }, [localFilters, setFilters, pageSize, fetchAllCities, searchPropertyAds]);

  useEffect(() => {
    if (hasFetched.current) {
      // Ensure status is PUBLISHED for public view if not explicitly set
      const searchFilters = {
        ...filters,
        status: filters.status || AdStatus.PUBLISHED,
        page: currentPage,
        limit: pageSize,
      };
      searchPropertyAds(searchFilters);
    }
  }, [currentPage, pageSize, filters, searchPropertyAds]);
  ...
  const handleApplyFilters = () => {
    const filtersWithStatus = {
      ...localFilters,
      status: AdStatus.PUBLISHED, // Always use PUBLISHED for public view
    };
    setFilters(filtersWithStatus);
    setCurrentPage(1);
    searchPropertyAds({ ...filtersWithStatus, page: 1, limit: pageSize });
  ...
  // Ads are already filtered by API (status: PUBLISHED), so use them directly
  const publishedAds = propertyAds || [];

  // For guests, show simple layout, for authenticated users show CustomerLayout
  const content = (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="mb-6 flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
        <button
          onClick={() => {
            const newFilters = { ...filters, adType: undefined, status: AdStatus.PUBLISHED };
            setLocalFilters({ ...localFilters, adType: undefined });
            setFilters(newFilters);
            setCurrentPage(1);
            searchPropertyAds({ ...newFilters, page: 1, limit: pageSize });
          }}
        ...
        {Object.entries(adTypeLabels).map(([value, label]) => (
          <button
            key={value}
            onClick={() => {
              const newFilters = { ...filters, adType: value as AdType, status: AdStatus.PUBLISHED };
            ...
            {publishedAds.map((ad) => {
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    if (isAuthenticated) {
                      router.push(`/property-ads/${ad.id}`);
                    } else {
                      router.push(`/property-ads/${ad.id}`);
                    }
                  }}
  ...
  // If authenticated, use CustomerLayout, otherwise use simple layout
  ...
  // For guests, show simple layout with login button
  return (
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xl font-bold text-gray-900">املاک یار</span>
            </div>
            <button
              onClick={() => router.push('/?tab=0')}
