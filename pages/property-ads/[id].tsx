...
import { FiArrowRight, FiImage, FiMapPin, FiPhone, FiHome, FiDollarSign, FiChevronLeft, FiChevronRight } from 'react-icons/fi';
...
export default function PropertyAdDetailPage() {
  ...
  if (isLoading) {
    ...
    return (
      <div className="flex min-h-screen flex-col bg-gray-50">
        ...
        <main className="flex-1">{loadingContent}</main>
      </div>
    );
  }
    ...
    const notFoundContent = (
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
        <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 px-4 py-12 text-center">
          ...
          <button
            onClick={() => router.push('/property-ads')}
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                {isRentAd && (
                  <div className="space-y-3">
                    {!ad.isPriceConvertible && ad.depositValue && (
                      <div>
                    ...
                    {!ad.isPriceConvertible && ad.monthlyRent && (
                      <div>
                    ...
                    {ad.isPriceConvertible && ad.minDepositValue && ad.maxDepositValue && (
                      <div>
                        ...
                        <p className="mt-2 text-xs font-medium text-green-600">قیمت قابل تبدیل است</p>
                      </div>
                    )}
                  </div>
                )}
  ...
  // For authenticated users, use CustomerLayout
  if (isAuthenticated && user) {
    return (
      <PrivateRoute>
        <CustomerLayout>
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <main className="flex-1">{detailContent}</main>
    </div>
  );
}
