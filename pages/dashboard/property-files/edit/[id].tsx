...
export default function EditPropertyFilePage() {
  ...
  useEffect(() => {
    if (selectedFile && !isFormInitialized) {
      setFormData({
        zone: selectedFile.zone,
        ...
        residential: selectedFile.residential ?? true,
        vacant: selectedFile.vacant ?? false,
        rentStatus: selectedFile.rentStatus ?? false,
        buildingAge: selectedFile.buildingAge,
        facade: selectedFile.facade,
        documentStatus: selectedFile.documentStatus,
        description: selectedFile.description,
        heating: selectedFile.heating ?? false,
        elevator: selectedFile.elevator ?? false,
        sauna: selectedFile.sauna ?? false,
        jacuzzi: selectedFile.jacuzzi ?? false,
        pool: selectedFile.pool ?? false,
        videoIntercom: selectedFile.videoIntercom ?? false,
        remoteDoor: selectedFile.remoteDoor ?? false,
        hasServantRoom: (selectedFile as any).hasServantRoom || false,
  ...
  const submitFormData = async (floorsOverride?: FloorDetails[]) => {
    ...
    // Validate id
    if (!id || typeof id !== 'string') {
      setSnackbar({ open: true, message: 'شناسه فایل نامعتبر است', severity: 'error' });
      return;
    }
    ...
    try {
      ...
      const dataToSanitize = {
        ...formData,
        floors: floorsOverride || formData.floors || [],
      };
      // Sanitize data before sending
      const sanitizedData = sanitizePropertyFileData(dataToSanitize);
      // Ensure floors is always included in the payload
      if (!sanitizedData.floors) {
        sanitizedData.floors = [];
      }
      ...
      const response = await updatePropertyFile(id, sanitizedData).unwrap();
      ...
      console.log('Field errors keys:', Object.keys(errors));
      ...
      console.log('=== ERROR HANDLING COMPLETE - NO REDIRECT ===');
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">کد یکتا</label>
                  <input
                    type="text"
                    value={selectedFile.uniqueCode || ''}
                    readOnly
