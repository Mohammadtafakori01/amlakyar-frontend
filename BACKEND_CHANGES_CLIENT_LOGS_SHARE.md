...
### 3.1. ویرایش مراجعه (Update Client Log)

**Endpoint:** `PATCH /client-logs/:id`

**دسترسی:** فقط `ADMIN` و `SECRETARY`

**توضیحات:** این endpoint اطلاعات یک مراجعه را به‌روزرسانی می‌کند.

**Request:**
```json
PATCH /client-logs/123e4567-e89b-12d3-a456-426614174000
...
2. بررسی دسترسی کاربر (فقط ADMIN و SECRETARY)
3. بررسی اینکه مراجعه متعلق به همان املاک کاربر است
4. به‌روزرسانی فیلدهای ارسال شده
5. بازگرداندن مراجعه به‌روزرسانی شده

---

### 3.2. حذف مراجعه (Delete Client Log)

**Endpoint:** `DELETE /client-logs/:id`

**دسترسی:** فقط `ADMIN`

**توضیحات:** این endpoint یک مراجعه را حذف می‌کند.

**Request:**
```
DELETE /client-logs/123e4567-e89b-12d3-a456-426614174000
```

**Response (200 OK):**
```json
{
  "message": "مراجعه با موفقیت حذف شد"
}
```

**خطاها:**
- `404 Not Found`: مراجعه یافت نشد
- `403 Forbidden`: کاربر دسترسی لازم را ندارد (فقط ADMIN)
- `400 Bad Request`: درخواست نامعتبر

**منطق:**
1. بررسی وجود مراجعه با ID داده شده
2. بررسی دسترسی کاربر (فقط ADMIN)
3. بررسی اینکه مراجعه متعلق به همان املاک کاربر است
4. حذف مراجعه از دیتابیس
5. بازگرداندن پیام موفقیت

---

### 3.3. اشتراک‌گذاری مراجعه (Share Client Log)
...
### 3.4. دریافت مراجعات عمومی (Get Public Client Logs)
...
### 3.5. به‌روزرسانی Endpoint دریافت مراجعات عادی
...
### 4.1. ویرایش مراجعه
- **ADMIN**: می‌تواند تمام مراجعات املاک خود را ویرایش کند
- **SECRETARY**: می‌تواند مراجعات املاک خود را ویرایش کند
- **سایر نقش‌ها**: دسترسی ندارند

### 4.2. حذف مراجعه
- **ADMIN**: می‌تواند تمام مراجعات املاک خود را حذف کند
- **سایر نقش‌ها**: دسترسی ندارند

### 4.3. اشتراک‌گذاری مراجعه
- **ADMIN**: می‌تواند تمام مراجعات املاک خود را به اشتراک بگذارد
- **SECRETARY**: می‌تواند مراجعات املاک خود را به اشتراک بگذارد
- **سایر نقش‌ها**: دسترسی ندارند

### 4.4. مشاهده مراجعات عمومی
...
### 5.1. قوانین ویرایش مراجعه
1. کاربر باید عضو همان املاکی باشد که مراجعه به آن تعلق دارد
2. تمام فیلدها اختیاری هستند (فقط فیلدهای ارسال شده به‌روزرسانی می‌شوند)
3. Validation فیلدها باید انجام شود (مثلاً شماره تلفن معتبر باشد)

### 5.2. قوانین حذف مراجعه
1. فقط ADMIN می‌تواند مراجعات را حذف کند
2. کاربر باید عضو همان املاکی باشد که مراجعه به آن تعلق دارد
3. حذف باید به صورت soft delete انجام شود (اختیاری) یا hard delete

### 5.3. قوانین اشتراک‌گذاری
...
### 5.4. قوانین مشاهده مراجعات عمومی
...
- [ ] پیاده‌سازی endpoint `PATCH /client-logs/:id` (ویرایش)
- [ ] پیاده‌سازی endpoint `DELETE /client-logs/:id` (حذف)
- [ ] پیاده‌سازی endpoint `PATCH /client-logs/:id/share` (اشتراک‌گذاری)
